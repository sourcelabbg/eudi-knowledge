---
name: "oid4vp-dcql"
description: "Use when writing or parsing DCQL (Digital Credentials Query Language) queries. Covers: credential queries, credential set queries, claims queries, trusted_authorities, claims path pointers for JSON and mdoc credentials, credential selection logic, and DCQL examples."
---
...
The Digital Credentials Query Language (DCQL, pronounced [ËdaklÌ©]) is a
...
Presentations matching the query.Â¶
A valid DCQL query is defined as a JSON-encoded object with the following
top-level properties:Â¶

        
credentials:
        REQUIRED. A non-empty array of Credential Queries as defined in Section 6.1
that specify the requested Credentials.Â¶

        

credential_sets:
        OPTIONAL. A non-empty array of Credential Set Queries as defined in Section 6.2
that specifies additional constraints on which of the requested Credentials to return.Â¶
...
multiple:
          OPTIONAL. A boolean which indicates whether multiple Credentials can be returned for this Credential Query. If omitted, the default value is false.Â¶
...
this object are defined per Credential Format. Examples of those are in Appendix B.3.5 and Appendix B.2.3. If empty,
...
trusted_authorities:
          OPTIONAL. A non-empty array of objects as defined in Section 6.1.1 that
...
one of the conditions present in the corresponding trusted_authorities array if present.Â¶
...
revealing information that would likely be rejected.Â¶
...
once in a single query. Wallets SHOULD ignore such duplicate claim queries.Â¶
...
The rules for selecting claims to send are defined in Section 6.4.1.Â¶

        

Multiple Credential Queries in a request MAY request a presentation of the same Credential.Â¶
...
types. How exactly the matching works is defined for the different types below.Â¶
...
be less likely to work due to the constraints on value matching (see Section 6.4.1 for more details).Â¶
Each entry in trusted_authorities MUST be an object with the following properties:Â¶

            
type:
            REQUIRED. A string uniquely identifying the type of information about the issuer trust framework.
Types defined by this specification are listed below.Â¶
...
and perform the matching logic for each provided value.Â¶
...
can be found in Section 15.10.Â¶
...
entire X.509 chain or parts of it.Â¶

            

Below is a non-normative example of such an entry of type aki:Â¶
...
Trusted List or its cascading Trusted Lists.Â¶

            

Below is a non-normative example of such an entry of type etsi_tl:Â¶
...
A valid trust path, including the given Entity Identifier, must be constructible from a matching credential.Â¶

            

Below is a non-normative example of such an entry of type openid_federation:Â¶
...
a particular use case with the Verifier.Â¶
Each entry in credential_sets MUST be an object with the following properties:Â¶
...
non-empty array of identifiers which reference elements in credentials.Â¶

          

required:
          OPTIONAL A boolean which indicates whether this set of Credentials is required
to satisfy the particular use case at the Verifier. If omitted, the default value is true.Â¶

        

Before sending the presentation request, the Verifier SHOULD display to the End-User the purpose, context, or reason for the query to the Wallet.Â¶


### 6.3. Claims Query
Each entry in claims MUST be an object with the following properties:Â¶
...
within the Credential, as defined in Section 7.Â¶
...
rules are defined in Section 6.4.1.Â¶
...
When conversion according to these rules is not clearly defined, behavior is out of scope of this specification.Â¶


### 6.4. Selecting Claims and Credentials
The following section describes the logic that applies for selecting claims
and for selecting credentials.Â¶
...
(see Section 15 for additional considerations). Wallets MUST NOT send
...
not selectively disclosable.Â¶


#### 6.4.1. Selecting Claims
The following rules apply for selecting claims via claims and claim_sets:Â¶
...
of format IETF SD-JWT VC).Â¶

            If claims is present, but claim_sets is absent,
the Verifier requests all claims listed in claims.Â¶
...
options, it MUST NOT return any claims.Â¶

            
              claim_sets MUST NOT be present if claims is absent.Â¶
...
values defined in Section 6.3, i.e.,
...
best-effort way to improve user privacy, but MUST NOT rely on it for security checks.Â¶
...
not intended to define options the End-User can choose from, see Section 6.4.3 for
...
deviate. Non-exhaustive examples of such reasons are:Â¶

scenarios where the Verifier did not order the options to minimize information disclosureÂ¶

            operational reasons why returning a different option than the first option has UX benefits for the Wallet.Â¶

          
If the Wallet cannot deliver all claims requested by the Verifier
according to these rules, it MUST NOT return the respective Credential.Â¶
...
since all claims are mandatory to present.Â¶


#### 6.4.2. Selecting Credentials
The following rules apply for selecting Credentials via credentials and credential_sets:Â¶

If credential_sets is not provided, the Verifier requests presentations for all
Credentials in credentials to be returned.Â¶

            
              Otherwise, the Verifier requests presentations of Credentials to be returned satisfyingÂ¶

all of the Credential Set Queries in the credential_sets array where the required attribute is true or omitted, andÂ¶

                optionally, any of the other Credential Set Queries.Â¶
...
they would not exist in the Wallet.Â¶
If the Wallet cannot deliver all non-optional Credentials requested by the
Verifier according to these rules, it MUST NOT return any Credential(s).Â¶
...
multiple of the sets of Credentials in options can satisfy the request.Â¶
...
processing are the referenced claims.Â¶


### 7.1. Semantics for JSON-based credentials
This section defines the semantics of a claims path pointer when applied to a JSON-based Credential.Â¶
...
is formed as follows:Â¶
Start with an empty array and repeat the following until the full path is formed.Â¶
...
non-negative, 0-based integer).Â¶

          To address all elements within an array, append a null value to the array.Â¶

        


#### 7.1.1. Processing
In detail, the array is processed from left to right as follows:Â¶

Select the root element of the Credential, i.e., the top-level JSON object.Â¶

            
              Process the query of the claims path pointer array from left to right:Â¶

If the component is a string, select the element in the respective
...
remove that element from the selection.Â¶

                If the component is null, select all elements of the currently
selected array(s). If any of the currently selected element(s) is not an
array, abort processing and return an error.Â¶

                If the component is a non-negative integer, select the element at
...
that array from the selection.Â¶

                If the component is anything else, abort processing and return an error.Â¶
...
The result of the processing is the set of selected JSON elements.Â¶
...
credential in ISO mdoc format.Â¶
...
element identifier.Â¶


#### 7.2.1. Processing
In detail, the array is processed as follows:Â¶

If the claims path pointer does not contain exactly two components or
one of the components is not a string then abort processing and return an error.Â¶

            Select the namespace referenced by the first component. If the namespace does
not exist in the mdoc then abort processing and return an error.Â¶

            Select the data element referenced by the second component. If the data element does not exist
in the Credential then abort processing and return an error.Â¶

          
The result of the processing is the selected data element value as CBOR data item.Â¶


### 7.3. Claims Path Pointer Example
The following shows a non-normative, simplified example of a JSON-based Credential:Â¶
...
claims:Â¶
            ...
            ["name"]: The claim name with the value Arthur Dent is selected.Â¶
...
selected.Â¶
            ...
            ["nationalities", 1]: The second nationality is selected.Â¶
...
first_name, and address.street_address:Â¶
...
Additional, more complex examples can be found in Appendix D.Â¶
